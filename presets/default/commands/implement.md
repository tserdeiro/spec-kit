...
That independence is what makes the verdict worth anything: a reused findings file is not a review. After closing the review, read `delivery.decision` from the close JSON: `proceed` continues to `gh pr ready`; `hold` means fix the `pending` findings (or, when `reason` is `inconclusive`, resolve the named causes) on the task branch and review the new candidate. Without `code-review`, fix what the findings name, on the task branch.
