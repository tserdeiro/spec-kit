...
- **Waiting.** Wait for the sub-agent's result. When the host returns control without it, wait again — no message, read, or query in between. Interrupt only for a new reason, after checking one piece of evidence once (did the branch head move, did the PR appear).
...
- **Communication.** Speak only to report a finding, a decision, or a blocker — never the absence of one. The completion report below stays as it is.
