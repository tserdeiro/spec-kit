...
def build_push_plan(
    ...
    *,
    ...
    if project is None:
        ...
        # The Project.description block carries only plain Source:/Plan:
    ...
    for task in desired.feature.tasks:
        ...
        # what comes back. Judge it by the hash on the marker's own open tag
        # -- a legacy separate body-hash comment (the pre-TDS-13 format)
        # does not match there, so it reads as absent and the Issue migrates
        # with one rewrite, same as a real mismatch. The hash covers only
        # the body, so an equal hash on a lone-marker layout (the TDS-12
        # format) would read as settled and never migrate -- the bounded
        # check forces that rewrite.
        description_changed = (
            _remote_marker_hash(issue.description, task.marker) != task.body_hash
            or not block_bounded(issue.description, task.marker)
        )
...
def _remote_marker_hash(content: str, marker: str) -> str:
    """The 12-hex hash carried on ``marker``'s own marker line, or "".

    Only that exact marker's own line counts -- open tag or, migrating from
    a 0.9.0-era remote, a lone trailing marker line; the same pattern
    matches either position. A legacy separate body-hash comment (the
    pre-0.9.0 bounded-block format, hash on its own line inside the body)
    is an unrelated comment and never matches here, so it reads as an
    absent hash -- see `build_push_plan`'s task-description branch for what
    that triggers.
    """
...
def _needed_content(remote_content: str, marker: str, desired_content_block: str, desired_hash: str) -> str | None:
    ...
    Idempotency here is judged by the hash on the block's own open tag,
    never by byte-comparing prose against `desired_content_block`: Linear
    normalizes markdown on write once a body has any markdown construct
    (inserts blank lines after HTML comments, rewrites `-` bullets to `*`),
    so our composed bytes are never the value Linear actually stores -- a
    byte compare would replan the same rewrite on every single push. This is
    not unique to Project.content: `build_push_plan`'s task-description
    branch judges every Issue's description the same way, for the identical
    reason. Trade-off, shared by both: a human edit made *inside* a managed
    block is invisible to this hash (it cannot see prose drift once Linear
    has already rewritten our bytes) and persists until the source artifact
    (the spec summary, or the task's own body) next changes. The feature's
    Project.description block has no such gap -- it carries only plain
    `Source:`/`Plan:` lines, which round-trip byte-identical, so it keeps
    self-healing byte-for-byte on every push, compared by
    `_needs_block_update` as before.
    """

    if desired_hash:
        # The bounded check mirrors the task-description gate: an equal
        # hash on an unbounded layout still needs its migrating rewrite.
        if _remote_marker_hash(remote_content, marker) == desired_hash and block_bounded(remote_content, marker):
            return None
        return merge_managed_block(remote_content, marker, desired_content_block)
    if marker_present(remote_content, marker):
