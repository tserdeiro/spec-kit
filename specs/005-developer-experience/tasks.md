...
- [x] T012 [US2] Linear `post_tool_use` handler in packages/spec-kit-linear
  ...
  - **Completion evidence**: PR #100 (base `005-T011-linear-session-start`, ready for review 2026-09-09); `uv run pytest packages/spec-kit-linear/tests -q` -> 437 passed, 187 subtests (the payload matcher table: `git push`, each `gh pr` verb, a chained command, `gh pr view`, `git pushd`, a non-Bash tool, invalid JSON, empty stdin); `grep -n -i reconcile presets/default/commands/implement.md` -> no match; the CLI's `validate_events` accepts both events; the shim from this repository's root reconciled on a `git push` payload and returned before any client on `gh pr view`, exit 0 and no stdout in both; `installed-artifact.sh` -> conformance passed; budget 120/120 (forecast ~60), at the stop exactly; fresh review closed `inconclusive`: zero blocking, 1 nit (the selector comment lost in the extraction, restored in 48d5073), and the reviewer did not reproduce the live shim run by instruction (it reaches this repository's real credentials) — the live dispatcher is T023's verification
