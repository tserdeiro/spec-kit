...
- [x] T013 [US2] `next_action` returns runnable commands in packages/spec-kit-linear
  ...
  - **Delivery**: single PR (~120 authored lines)
  - **Completion evidence**: PR #101 (base `005-T012-linear-post-tool-use`, ready for review 2026-09-09); `uv run pytest packages/spec-kit-linear/tests -q` -> 444 passed, 203 subtests, including the exhaustive state × source contract test (every return starts with `/speckit.` or is the wait sentence), the removed work-item `unstarted` row, `PullRequest.number` parsing, the number threading for tasks and work items, and the feature header; the source tree's `status --current` on this feature -> `Feature 005: Feature Specification: Developer experience` and every NEXT a command or the wait sentence; `installed-artifact.sh` -> conformance passed; budget 142/240 (forecast ~120); fresh review closed `no-blocking-findings` with 1 major (dead `checked`/`task` parameters kept in `next_action`) and 1 minor (no direct test for the task-side number threading), both fixed in dafadde
