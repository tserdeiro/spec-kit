...
- [x] T014 Transversal verification: SC-001…SC-006 evidence on the integrated feature branch
  ...
  - **Completion evidence**: feature merged to `main` (#43, merge commit `48e4039`, 2026-09-02) and published (`ad81539`: linear 0.11.0, code-review 0.3.0, preset 0.8.0, bundles 0.14.0; `--published` conformance passed at the consistent point). SC-001: every product phase of this feature ended committed with no hook announcement (`dd070e8`…`d4ed0a5`, clean `git status` after each). SC-002: TDS-14…32 moved through In Progress/review/Done by the loop's `push --hook` alone — no human push. SC-003: unlinked and placeholder configs name `onboard` pre-network (0.11.0 suite, 410 tests); the doctor names `deleteBranchOnMerge`/`mergeCommitAllowed`; `onboard` surfaced the missing workspace GitHub integration. SC-004: the parser fixture built from the template's instructive section parses (0.11.0 tests); every review of T001–T019 closed on the first attempt with the documented findings shape. SC-005: the gate-in-implement rule ships in preset 0.8.0 (bundles conformance green); this feature's own gate predates it, so the first from-scratch run is the next feature's evidence. SC-006: the conformance `trunk` scenario resolves a configured `trunk: release` ≠ GitHub default (green in CI and locally); app-maker sets `trunk: dev`. Consumer upgrade: this repository's installed extensions upgraded to 0.11.0/0.3.0 (vendored `src` byte-identical to the tags); app-maker bundles updated to 0.14.0 in its PR
...
- [x] T018 Fix the findings session bind in spec-kit-code-review: separate normalized output, exact-path bind, real doc-parity test
