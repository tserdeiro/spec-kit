...
- **Decision**: `trunk: <branch>` in the consumer's
  `.specify/extensions/git/git-config.yml`; one stdlib-only preset helper
  parses its top-level one-line string/null value, rejects duplicate or
  unsupported `trunk` forms, and validates the explicit trunk or
  GitHub-default fallback with Git. The feature-PR path and first-task
  implement refresh invoke the installed helper. Documented in the preset
  and root READMEs.
