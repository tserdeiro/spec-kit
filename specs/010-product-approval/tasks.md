...
- [x] T004 [US3] Enforce the product gate at task entrypoints and verify adversarial drift in presets/default/scripts/python/task_base.py
  ...
  - **Delivery**: single PR (~390 authored lines)
  - **Completion evidence**: Review fixes validate the lexical feature path, the exact fetched stack commit, and adversarial drift. Focused gate/task/PR tests: `uv run --frozen --offline --project packages/spec-kit-linear pytest presets/default/tests/test_product_gate.py presets/default/tests/test_task_base.py presets/default/tests/test_pr_create.py -q` -> 73 passed. Installed-consumer conformance: `bash scripts/conformance/bundles.sh` -> `conformance passed` (all bundles). Full preset suite: `uv run --frozen --offline --project packages/spec-kit-linear pytest presets/default/tests -q` -> 128 passed in 65.04s. `git diff --check` clean.
