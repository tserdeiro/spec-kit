...
- [x] T004 [US3] Enforce the product gate at task entrypoints and verify adversarial drift in presets/default/scripts/python/task_base.py
  ...
  - **Delivery**: single PR (~390 authored lines)
  - **Completion evidence**: Focused gate/task/PR tests: `uv run --frozen --offline --project packages/spec-kit-linear pytest presets/default/tests/test_product_gate.py presets/default/tests/test_task_base.py presets/default/tests/test_pr_create.py -q` -> 54 passed. Installed-consumer conformance: `bash scripts/conformance/bundles.sh` -> `ok: product`, `developer`, `reviewer`, `coexist`, `update`, `trunk`. Full preset suite: `uv run --frozen --offline --project packages/spec-kit-linear pytest presets/default/tests -q` -> 109 passed. `git diff --check` clean.
