...
- [x] T002 [US2] Complete repeatable feature publication and handoff in presets/default/commands/pr.md
  ...
  - **Completion evidence**: Implementation 32f3e9b, including reviewed publication retry/failure handling, every push target, native branch namespaces, gate identity, scoped staging, and stable Linear evidence. Product command tests: 8 passed; full preset suite: 79 passed; installed bundle conformance passed with synthetic providers. Budget: 139/400 authored executable lines; diff check clean. Actual-agent acceptance follows in T006; PR candidate review and CI gate readiness.
...
- [x] T003 [US3] Validate published product content before implement in presets/default/scripts/python/product_gate.py
  ...
  - **Completion evidence**: Implementation 2feafc8, b5c8b01, 208d663, 9efe7ae, e550070, 4979d4f, f9cede0, and c3537ec, including independent-review corrections for feature refs, unknown task fields, symlinked parents, missing-gate recovery, checkout identity, divergent push destinations, host-qualified push identity, required-artifact symlinks, stale OID recovery, unreadable push recovery, and exact namespaced continuation. Focused gate/command tests: 36 passed; complete preset suite: 107 passed in three passing groups; installed bundle conformance passed with synthetic providers; final helper observation from an explicit feature selection against feature PR #147 passed. Budget: 639/400 authored executable lines under the user-authorized T003 exception; diff check clean. PR readiness requires fresh independent review and green CI.

- [x] T004 [US3] Enforce the product gate at task entrypoints and verify adversarial drift in presets/default/scripts/python/task_base.py
  ...
  - **Depends on**: T003
  - **Boundaries**: Call the existing `product_gate.py` check before mutation in `task_base.py` refresh/task and `pr_create.py` task; update `tests/{test_task_base,test_pr_create,test_product_gate,conftest}.py`. Keep work-item behavior independent. Cover added/deleted artifacts, missing files, file-mode/symlink substitutions, index conflicts, moving head/base/state, stale remote refs, stacked completion progress, multiline evidence, task additions/renames, every immutable ledger field/forecast, fenced samples, and malformed blocks. Adapt the existing installed-consumer Git/gh fixtures in `scripts/conformance/bundles.sh` in this same task so every pre-existing valid task case supplies its published gate; preserve deliberate invalid cases. Retain the existing task-base and PR-base rules after a valid gate; diagnostics specify product closure actions.
  - **Evidence**: `uv run --frozen --offline --project packages/spec-kit-linear pytest presets/default/tests/test_product_gate.py presets/default/tests/test_task_base.py presets/default/tests/test_pr_create.py -q` -> all invalid entrypoints stop before commit/checkout/branch/push/PR/Linear mutation; valid task and independent work-item paths retain their expected behavior; `bash scripts/conformance/bundles.sh` passes with the adapted fixtures before T005 adds broader coverage.
  - **Delivery**: single PR (~390 authored lines)
  - **Completion evidence**: Final review fixes establish the repository boundary before lexical inspection, reject internal aliases while preserving legitimate external path aliases, and retain exact fetched stack validation plus adversarial drift coverage. Focused gate/task/PR tests: `uv run --frozen --offline --project packages/spec-kit-linear pytest presets/default/tests/test_product_gate.py presets/default/tests/test_task_base.py presets/default/tests/test_pr_create.py -q` -> 75 passed. Installed-consumer conformance: `bash scripts/conformance/bundles.sh` -> `conformance passed` (all bundles). Full preset suite: `uv run --frozen --offline --project packages/spec-kit-linear pytest presets/default/tests -q` -> 130 passed in 57.64s. `git diff --check` clean.
