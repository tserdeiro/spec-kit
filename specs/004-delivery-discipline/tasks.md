...
- [x] T002 [US1] One linear stack: base selection in implement-append.md, stack-aware task base in presets/default/commands/pr.md, ledger rule in tasks-append.md
...
- [x] T004 [US1] Branch identity check in presets/default/commands/pr.md
  ...
  - **Completion evidence**: PR #68 (ready 2026-09-03, stacked on #67 — base `004-T003-stack-propagate`; opened by the new block itself, which matched branch T004 against the ledger's first unchecked task); review budget counted 63 authored executable lines (`bundles.sh`; Markdown excluded as D9 defines) against the ~50 forecast; `bash scripts/conformance/bundles.sh` passed with the mismatch (exit 2, no gh call, both tasks named), named-task-wins, missing-ledger, and fence cases; the awk agrees with `spec_kit_linear.parser` on the template and the fixture; `sh -n`/`dash -n` on the rendered block; `git diff --check` clean; fresh Sonnet review (session `8ee71164`) found 3 minor — backtick info-string rule not mirrored and missing-ledger diagnostic, fixed in-branch; PR-body line count contradicting D9's definition, corrected in the body; verdict no-blocking-findings

- [x] T005 [US1] Closure rules: the loop never merges, root-first merging, on-request merge with worktree prune, and the revert path in implement-append.md and README.md
  ...
  - **Completion evidence**: PR #69 (ready 2026-09-03, stacked on #68 — base `004-T004-branch-identity`); text only, 0 executable lines counted by the review budget (43 net Markdown lines against the ~80 forecast); `bash scripts/conformance/bundles.sh` passed; `git diff --check` clean; the installed implement skill names the root-first rule, the on-request merge path with `git worktree prune`, and the `revert(scope): <subject>` commit; fresh Sonnet review (session `d3491bcc`) found 1 minor + 2 nit — the rationale overstated that a retarget triggers no workflow (the naming check listens to `edited`; tests and conformance do not), the revert placeholder rendered three ways, a sentence duplicated between the preset README sections — all fixed in-branch; verdict no-blocking-findings
...
- [x] T006 [US2] Budget stop at twice the forecast in presets/default/commands/implement-append.md and the forecast rule in tasks-append.md
  ...
  - **Completion evidence**: PR #70 (ready 2026-09-03, stacked on #69 — base `004-T005-closure-rules`); the block ran on its own task before the PR and before ready: `budget: 77/120 (forecast ~60)`; `bash scripts/conformance/bundles.sh` passed with the `budget` scenario (under with excluded suffixes, binaries and a spaced path; over; no forecast → 400; forecast whose double exceeds 400; first task behind the fenced sample); `sh -n`/`dash -n` on the rendered block; `git diff --check` clean; orchestrator review: forecast awk fence-aware (shared with the identity check), fixed in-branch; fresh Sonnet review (session `1b0b9ce1`) found 1 major (numstat split by whitespace broke on renames and spaced paths — now `--no-renames` and `awk -F'\t'`), 2 minor, 1 nit — the ready-for-review re-check anchored at the action and the ledger guard added; the configurable `budget.limit` ceiling accepted as-is (plan D9 fixes 400; no consumer configures it); verdict no-blocking-findings

- [x] T007 [US2] Engineering principles in the base review rules: RULE_TEMPLATE in packages/spec-kit-code-review/src/spec_kit_code_review/doctor.py, this repository's .opencodereview/rule.json, and the AGENTS.md single source
  ...
  - **Delivery**: single PR (~60 authored lines)
  - **Completion evidence**: PR #71 (ready 2026-09-04, stacked on #70 — base `004-T006-budget-stop`); budget-stop `42/120 (forecast ~60)`; `uv run pytest packages/spec-kit-code-review/tests -q` → 789 passed, 494 subtests (5:08); `.opencodereview/rule.json` byte-identical to `RULE_TEMPLATE` (diff empty); `CLAUDE.md` = `@AGENTS.md`, `AGENTS.md` carries the dogfooding exception; changelog entry under the H1 where `publish.sh --bump` inserts the next heading; `git diff --check` clean; fresh Sonnet review (session `e7202936`, packet 124 KB reviewed file by file) found 1 minor — no automated tie between the committed rule file and the template — fixed in-branch with a skip-if-absent test (53 passed in `test_doctor.py`); verdict no-blocking-findings
...
- [x] T008 [US3] `protected_paths` deterministic finding in packages/spec-kit-code-review
  ...
  - **Completion evidence**: PR #72 (ready 2026-09-04, stacked on #71 — base `004-T007-review-principles`); `uv run pytest packages/spec-kit-code-review/tests -q` → 799 passed, 510 subtests (5:13) on the candidate, plus the three touched suites green after the last simplification; budget-stop `271/280 (forecast ~140)` before the PR and 272/280 before ready — 1.9×, under the stop line, tests are 160 of those lines; `git diff --check` clean; orchestrator review: the deletion anchor parsed `git diff --unified=0` for a line that is always 1 — helper removed; fresh Sonnet review (session `40bb4130`, packet 157 KB reviewed file by file) found 1 major — a third, untested anchor branch (RIGHT 1..1 for in-file removals) — collapsed to two branches (RIGHT hunk, else LEFT 1..1) and tested; verdict no-blocking-findings
...
- [x] T015 [US4] Project at plan: `tasks.md` optional in the packages/spec-kit-linear parser and projection
  ...
  - **Delivery**: single PR (~70 authored lines)
  - **Completion evidence**: PR #74 (ready 2026-09-04, stacked on #73 — base `004-T009-linear-worktree`); budget-stop `60/140 (forecast ~70)`; `uv run pytest packages/spec-kit-linear/tests -q` → 423 passed, 176 subtests; `push --hook --dry-run --json` on a ledger-less feature exits 0 with `project.create` only and the `tasks_pending` diagnostic; a missing `plan.md` still fails `artifact_missing`; `git diff --check` clean; fresh Sonnet review (session `bfe4ce4a`, packet 147 KB reviewed file by file) found 1 major (a `has_ledger` field redundant with `phases`, since an empty ledger already fails in the parser) and 1 minor (duplicated diagnostic block) — both fixed in-branch; verdict no-blocking-findings

- [x] T010 [US4] Published-asset digests in scripts/conformance/bundles.sh `--published`
  ...
  - **Delivery**: single PR (~90 authored lines)
  - **Completion evidence**: PR #75 (ready 2026-09-04, stacked on #74 — base `004-T015-project-at-plan`); budget-stop `81/180 (forecast ~90)`; `bash scripts/conformance/bundles.sh --published` on the published tree → `ok: linear published digests`, `ok: code-review published digests`, conformance passed; default mode unchanged and passed; throwaway-clone checks: a flipped `release_zip_sha256` fails naming the asset and both digests, a nonexistent release URL prints the pending line and passes, an invalid host fails with HTTP 000, a deleted tag names `git fetch --tags origin`; `git diff --check` clean; design correction found by the implementer: `publish.sh` runs `--published` before pushing, so plan D13's post-publication claim was wrong — HTTP 404 on the zip is reported as pending and the maintainer's post-publication run verifies it (D13 corrected in T012); fresh Sonnet review (session `39ef04bd`) found 1 minor (git recomputations not id-scoped on failure) and 1 nit (same ok line when pending) — both fixed in-branch; verdict no-blocking-findings
...
- [x] T013 Release preparation: `publish.sh --bump preset=0.9.0 linear=0.12.0 code-review=0.4.0 bundles=0.15.0`
  ...
  - **Completion evidence**: PR #78 (ready 2026-09-04, stacked on #77 — base `004-T012-round-docs`); generated bump only (14 files, +24/−20; budget-stop `17/40`); the `- TODO` skeletons replaced by the round's changelog bullets; the preset README install URL updated by hand (the script does not rewrite it); default conformance passed on the bumped tree; linear 423 passed, code-review 799 passed, 511 subtests; `--published` exits 1 with exactly the four catalog-parity lines expected before publication; `git diff --check` clean; fresh Sonnet review (session `345a0879`, packet 139 KB reviewed file by file) found nothing — verdict no-blocking-findings; first publication: human, from `main`, after #64 merges

- [x] T014 Transversal verification: SC-001…SC-010 evidence on the integrated feature branch in specs/004-delivery-discipline/tasks.md
  ...
  - **Completion evidence**: PRs #65 -> #66 -> #67 -> #68 -> #69 -> #70 -> #71 -> #72 -> #73 -> #74 -> #75 -> #76 -> #77 -> #78 (T001, T002, T003, T004, T005, T006, T007, T008, T009, T015, T010, T011, T012, T013) each based on its predecessor's head branch, root #65 on `004-delivery-discipline` (`gh pr list --json number,headRefName,baseRefName`); the ledger at the top of #78 shows all fourteen delivered tasks checked (`grep -c '^- \[x\]' specs/004-delivery-discipline/tasks.md` = 14) and no fix was propagated by hand — `git log main..HEAD` carries no `merge(task): carry the T### fix into T###` commit, since every review fix landed on its own task branch before the next task's branch forked; the `stack-propagate` block never had to run this round (SC-001). SC-002: all fifteen PRs of the stack, the still-draft feature PR #64 included, show `mergedAt: null` (`gh pr view <n> --json mergedAt,state`); the fourteen task PRs are each ready (non-draft) with its fresh review session closed (`9712e67f`, `35526323`, `2cee6014`, `8ee71164`, `d3491bcc`, `1b0b9ce1`, `e7202936`, `40bb4130`, `0a08609b`, `bfe4ce4a`, `39ef04bd`, `b9f57c64`, `c2c1dd62`, `345a0879`). SC-003: T008's suite (#72) proved a modified, added, and deleted protected path each raise the blocking `contract` finding and `changes-requested` on a `004-…` base, and none on a `main` base; this repository consumes the check itself only once code-review 0.4.0 is installed (C-005). SC-004: the `budget-stop` block fired on T009 at 231/180 and returned the task to the human; T011's implementer measured 213/180 and stopped before the block ran, applying the T009 precedent directly; both fit after consolidating duplicated test fixtures, never by widening the forecast — both Delivery lines keep their original `~90` forecast, and the `docs(specs): check T0##` commits sampled (T009, T011) touch only the checkbox and the Completion evidence line (2 insertions/2 deletions each). SC-005: T010's `--published` run verified the release-zip, subtree-archive, and manifest digests of both extensions against `versions.lock.yml` on the published tree, and failed naming the asset and both digests against a throwaway clone with one digest flipped. SC-006: the `skill-mirror` and `ignore-entries` blocks, each run twice on this two-agent repository by the orchestrator (report, then fix), reported `mirror: nothing to do` and `ignore: nothing to do` both times; the `bundles.sh` fixture proves the idempotency underneath — a byte-exact rebuild, then a second fix run that also reports nothing to do. SC-007: T009's and T015's `git init` + `git worktree add` fixtures resolve the main checkout's config and env with no worktree-local files; the linear suite reached 423 passed after T015 (419 at T009). SC-008: the orchestrator's own `implement` run of this round printed no hook announcement — the optional git-commit and code-review hooks were skipped silently under the appended rule, and the mandatory Linear hook ran — and no task or `docs/dogfooding.md` entry records one; raw session transcripts are not repository artifacts, so the witness is the orchestrator's transcript, not a file. SC-009: the `ignore-entries` block reported nothing to do on this repository, whose ignore file already covers the cache and venv paths; the `bundles.sh` fixture adds the two cache entries in one fix run and changes nothing on the next. SC-010: T015's `push --hook --dry-run --json` on a ledger-less feature exits 0 with `project.create` only and the `tasks_pending` diagnostic; the live proof — a fresh feature's Project at `plan`, before `tasks.md` — is deferred to the next feature once linear 0.12.0 is published and installed (A-002). Pending trunk chores, not this feature: upgrade this repository's installed extensions to linear 0.12.0 / code-review 0.4.0 once published, and flip `docs/dogfooding.md`'s *entregada* statuses to *resuelta*.
