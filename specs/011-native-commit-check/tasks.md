...
- [x] T007 Detect manual validator invocation behind the Husky dispatcher per FR-007 and FR-008 (partial)
  ...
  - **Completion evidence**: Correction round: focused native hook tests: 33 passed; supplied `/private/tmp/t007-review-repro.py` against the candidate reports custom Husky and hidden Lefthook duplicates refused, with a pre-push-only validator left non-duplicate and installable; installed Git 2.54/2.55 × plain/Husky 9.1.7/Lefthook 2.1.12 matrix: `native installed commit-msg conformance passed`, including `git254-husky` and `git255-husky` manual-validator refusal with unchanged files; package suite: 1038 passed, 639 subtests passed; `bash -n packages/spec-kit-code-review/scripts/conformance/commit-msg.sh && git diff --check`: clean. Logs: `/private/tmp/011-T007-package-final2.log`, `/private/tmp/011-T007-conformance-final2.log`.
