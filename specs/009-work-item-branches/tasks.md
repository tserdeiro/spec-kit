...
- [ ] T001 [US1] Read Issue context and native branch resolution in packages/spec-kit-linear/src/spec_kit_linear/linear_client.py
  ...
  - **Completion evidence**: Original native-read slice: PR #144; package suite 509 passed / 286 subtests; `git diff --check` passed; budget 181/400. Fresh extension review of `028fd1812c90c6d78d8171c208e7487b5288fa84`: `no-blocking-findings`, 0 findings, 6 covered ranges / 0 gaps, session closed. Independent read-only live probe: api_key credentials, context present, exact Git-valid native branch, duplicate inputs resolved once to the same Issue; observed prefix is literal in the configured template, without viewer-prefix inference. Batched Issue-context extraction and a fresh full-candidate review remain pending.
