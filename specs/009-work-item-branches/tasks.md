...
- [x] T003 [US1] Start configured and unconfigured work items in presets/default/scripts/python/work_item_start.py
  ...
  - **Delivery**: single PR (~340 authored lines)
  - **Completion evidence**: PR #158; preset suite 85 passed, focused work-item suite 14 passed; py_compile and `git diff --check` passed; budget 376/400. Fresh review of `64ca55b4a18473ebb76b8bec2004192eb55263ce` closed with `no-blocking-findings`, 1 minor usage-contract finding, 6 covered ranges / 0 gaps. Usage corrected in `2c6619a`; independent no-argument check exits 2 with issue-key/title guidance. Reviewer test rerun was unavailable because pygments was absent from its offline cache; original suite evidence remains valid. Literal @{-1}, reserved names, selected local/main/global configuration without resolver, and broken YAML fixtures pass. Bundle caller regression corrected in `4f91bc7`; full `bash scripts/conformance/bundles.sh` and 23 targeted preset tests passed. Non-fatal offline reconciliation warning (pygments unavailable) is not live synchronization evidence. Installed runtime acceptance follows in T008.
...
- [x] T008 [US3] Complete PR routing and installed contract evidence in presets/default/commands/pr.md
  ...
  - **Delivery**: single PR (~350 authored lines)
  - **Completion evidence**: Linear package suite 563 passed / 316 subtests; code-review package suite 1013 passed / 621 subtests; preset suite 112 passed; installed-artifact, reviewer, and role-bundle conformance passed; `git diff --check` and shell syntax checks passed. Installed consumers ran task-base work-item setup and exact-head adoption, PR identity/base routing with canonical Tracker evidence, unconfigured fallback, and native/unresolved/conflict status/push cases against query-only GraphQL; unresolved and conflicting identities preserved zero speculative operations. Reviewer conformance regenerated `speckit-pr` through supported dev install and verified no Linear bridge. Read-only live bridge probes resolved TDS-108 and its branch lookup to the same exact native branch `feature/tds-108-t008-complete-pr-routing-and-installed-contract-evidence-in`; `git check-ref-format --branch` passed; credential-prefix shape `feature/<redacted>`; no remote mutation. Friction recorded in `/private/tmp/spec-kit-009-extra-round-friction.md`.
