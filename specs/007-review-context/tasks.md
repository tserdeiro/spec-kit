...
- [x] T001 [US1] Expose complete task blocks in packages/spec-kit-code-review/src/spec_kit_code_review/sdd_context.py
  ...
  - **Completion evidence**: PR #127; full package suite: 855 tests and 423 subtests passed; diff check passed. Independent review found nested checklist misclassification; fixed in `38d0ece` with regression coverage. Review evidence: https://github.com/tserdeiro/spec-kit/pull/127#issuecomment-5642251766. Independent reviews also found duplicate-ID, missing-dependency, and required-field gaps; all corrected with regression tests. Complete reached blocks now render after artifact truncation; final focused run: 65 tests and 2 subtests passed, golden passed. Final candidate review and CI are recorded in the PR.
